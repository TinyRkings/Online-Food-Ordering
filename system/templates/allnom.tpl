   ...
   {section name=nom_id loop=$nomarr}
    ...
    <table width="636" height="134" border="0" cellspacing="0" cellpadding="0">
    	<tr>
            ...
            <td height="35">菜单名称：{$nomarr[nom_id].name}</td>
            <td width="156" height="35">商品类别：{$nomarr[nom_id].class}</td>
    	    <td width="157">菜单类型：{$nomarr[nom_id].model}</td>
    	</tr>
    	<tr>
    	  <td height="23">菜单品牌：{$nomarr[nom_id].brand}</td>
  	      <td height="23" colspan="2">菜单产地：{$nomarr[nom_id].area}</td>
    	</tr>
    	<tr>
    	  <td width="178" height="23">供应量：{$nomarr[nom_id].stocks}</td>
  	      <td colspan="2">销售量：{$nomarr[nom_id].sell}</td>
    	</tr>
    	<tr>
